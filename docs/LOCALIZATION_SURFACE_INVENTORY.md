...
| Telegram command menu | `hermes_cli.commands_platforms.telegram_menu_commands()` → adapter `set_my_commands` | Known descriptions are localized and configured commands are hidden for default/private/group scopes. |
...
| Gateway restart/shutdown notifications | Lifecycle send paths | Intentionally left outside localization: the plugin must not import `gateway.run` or monkey-patch lifecycle methods during registration. |
